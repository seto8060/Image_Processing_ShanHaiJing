...
# Region-aware Adaptive Instance Normalization (RAIN) Module
class RAIN(nn.Module):
    """
    Region-aware Adaptive Instance Normalization (RAIN) Module
    CVPR 2021: https://github.com/junleen/RainNet
    """
    def __init__(self, dims_in, eps=1e-5):
        ...
        self.fg_gamma = nn.Parameter(torch.zeros(dims_in),requires_grad=True)
        self.fg_beta = nn.Parameter(torch.zeros(dims_in),requires_grad=True)
        self.bg_gamma = nn.Parameter(torch.zeros(dims_in),requires_grad=True)
        self.bg_beta = nn.Parameter(torch.zeros(dims_in),requires_grad=True)
        self.eps = eps
    def forward(self, x, mask):
        """
         前景区域自适应归一化，背景区域普通归一化
        """
        # character map's size is changing
        mask = F.interpolate(mask.detach(), size = x.size()[2:],mode = 'nearest')

        # unstyle
        fg_mean,fg_std = self.get_mean_std(x*mask,mask)
        unstyle_fg =self.normalize(x ,fg_mean,fg_std)
        # restyle
        bg_mean,bg_std = self.get_mean_std(x*(1-mask),1-mask)
        restyle_fg = unstyle_fg * bg_std+bg_mean
        normalized_bg = self.normalize(x*(1-mask) ,bg_mean,bg_std)
        adjusted_bg = (normalized_bg * (1+self.bg_gamma[None,:,None,None])+self.bg_beta[None,:,None,None]) *(1-mask)
        # affine transform
        adjusted_fg = (restyle_fg*(1+self.fg_gamma[None,:,None,None])+self.fg_beta[None,:,None,None]) *mask
    ...
    def get_mean_std(self,region,mask):
        ...
        mean = (sum/(num+self.eps))[:,:,None,None]
        var = torch.sum(mask*(region-mean)**2,dim=[2,3])/(num+self.eps)
        var = var[:,:,None,None]
...
def create_norm_layer(norm_type = 'instance'):
...
def define_G(input_nc,output_nc,ngf,netG,norm='batch',use_dropout=False,
    ...
    Parameters:
        ...
        norm (str)          -- the normalization layer: batch | instance | none
        use_dropout (bool)  -- if use dropout layers
        init_type (str)     -- the name of our initialization method.
        init_gain (float)   -- scaling factor for normal, xavier and orthogonal.
        gpu_ids (list)      -- which GPU devices to use

    Returns a generator network.
    ---
    This is the docstring from the original code.Actually, we might just use one gpu while only unet_256 is implemented.
    """
    norm_layer = create_norm_layer(norm_type=norm)

    net = RainNet(input_nc,output_nc,ngf,norm_layer,use_dropout, use_attention = True)
...
def define_D(input_nc,ndf,netD,n_layers_D = 3,norm ='batch',init_type= 'normal',init_gain=0.02, gpu_ids=[]):
    ...
    norm_layer = create_norm_layer(norm_type=norm)
    if netD == 'basic':
        net = NLayerDiscriminator(input_nc, ndf, 3, norm_layer)
    elif netD == 'n_layers':
        net = NLayerDiscriminator(input_nc, ndf, n_layers_D, norm_layer)
    elif netD == 'pixel':
        net = PixelDiscriminator(input_nc, ndf, norm_layer)
    else:
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    """Initialize a network: put it on GPU and initialize its weights
...
def get_act_conv(act,dims_in,dims_out,kernel,stride,padding,bias):
    conv = [act]
    conv.append(nn.Conv2d(dims_in,dims_out,kernel,stride,padding,bias=bias))
    return nn.Sequential(*conv)

def get_act_dconv(act,dims_in,dims_out,kernel,stride,padding,bias):
    dconv = [act]
    dconv.append(nn.ConvTranspose2d(dims_in,dims_out,kernel,stride,padding,bias=bias))
    return nn.Sequential(*dconv)
...
class RainNet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=RAIN, 
        ...
        norm_type_list = [create_norm_layer('instance'),norm_layer]
...
class UnetBlockCodec(nn.Module):
    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None, submodule=None, outermost=False, innermost=False,
        ...
        if outermost:
            ...
            self.upnorm = norm_layer(outer_nc) if dec else create_norm_layer('instance')(outer_nc)
        else:
            self.down = nn.Sequential(
                nn.LeakyReLU(0.2, True),
                nn.Conv2d(input_nc, inner_nc, kernel_size=4, stride=2, padding=1, bias=use_bias),
            )
            self.downnorm = norm_layer(inner_nc) if enc else create_norm_layer('instance')(inner_nc)
            self.submodule = submodule
            self.up = nn.Sequential(
                nn.ReLU(True),
